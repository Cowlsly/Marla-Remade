...
class CastRtpPacketizerTest {

    private fun packetizer(maxPacketSize: Int = MAX_RTP_PACKET_SIZE) =
        CastRtpPacketizer(PAYLOAD_TYPE, SSRC, maxPacketSize, sequenceNumberStart = 0x1234)
    ...
    fun `the sequence number wraps at sixteen bits`() {
        val p = CastRtpPacketizer(PAYLOAD_TYPE, SSRC, sequenceNumberStart = 0xffff)
