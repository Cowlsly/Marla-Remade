...
class CastCryptoTest {
    ...
    fun `the iv is the frame id big-endian at offset eight, xored with the mask`() {
        ...
        val crypto = CastCrypto(key, mask)
    ...
    fun `a key of the wrong length is refused rather than silently truncated`() {
        ...
        assertFailsWith<IllegalArgumentException> { CastCrypto(ByteArray(32), mask) }
        assertFailsWith<IllegalArgumentException> { CastCrypto(key, ByteArray(12)) }
    }
    ...
    fun `an empty payload encrypts to nothing rather than throwing`() {
        ...
        assertTrue(CastCrypto(key, mask).crypt(FrameId(3), ByteArray(0)).isEmpty())
    }
