...
internal const val RTP_MARKER_BIT = 0b1000_0000
internal const val CAST_KEY_FRAME_BIT = 0b1000_0000
internal const val CAST_HAS_REFERENCE_FRAME_ID_BIT = 0b0100_0000

/**
 * One encoded, already-encrypted frame - either about to be split into packets, or just
 * reassembled from them.
 *
 * The same type on both ends deliberately: [RtpPacketizer] turns one of these into packets and
 * [FrameAssembler] turns packets back into one, so the round-trip assertion is a single `==`.
 */
...
/**
 * Splits an encrypted frame into RTP packets.
 *
 * The wire format is openscreen's `cast/streaming/impl/rtp_packetizer.cc` (`GeneratePacket`), with
 * the field diagram in `impl/rtp_defines.h`:
 *
 * ```
 *  0                   1                   2                   3
 ...
 * ```
 *
 * **Kept byte-identical to Cast Streaming even though nothing requires it any more.** Both ends of
 * this protocol are ours, so the format is now only a choice - but it is a written, golden-byte
 * tested choice with a matching depacketizer, and inventing a second one would buy nothing.
 *
 ...
 *
 * The adaptive-latency extension is never emitted - it only carries a mid-stream playout-delay
 * change, which nothing here asks for - but its 4 bytes stay reserved so a frame never has to be
 * re-split. [RtpDepacketizer] refuses a packet that does declare extensions, since we never write
 * one and a peer that did would not be us.
 */
class RtpPacketizer(
