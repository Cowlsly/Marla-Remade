...
`speech` **no longer downloads a recogniser**, though TTS voices are still fetched:

- **Speech-to-text** is `whisper-tiny` int8 ONNX (~40 MB) in `speech/src/main/assets/whisper-tiny/`, run on `onnxruntime-android` by `WhisperOnnxEngine.kt`. It replaced a 117 MB runtime download of the ncnn conversion from `data.vayunmathur.com/models/whisper-tiny/`. Upstream is **`onnx-community/whisper-tiny`** on HuggingFace (an ONNX Runtime dynamic quantization of `openai/whisper-tiny`, Apache-2.0) — a **build-host** dependency only, pinned by SHA-256 and refreshed by `scripts/speech/fetch_whisper_onnx.sh`. If it vanished, shipped builds keep transcribing. Note this moved the log-mel front end and the KV-cached decode loop into Kotlin (`WhisperFeatures.kt`, `WhisperOnnxEngine.kt`), because `onnx2ncnn` supports none of the quantized ops and the ncnn AAR expects a six-net decomposition HuggingFace does not export.
- **Text-to-speech** voices all remain SHA-256-pinned downloads from `data.vayunmathur.com/models/piper/` (29 languages, 28–80 MB each). Bundling the English one was considered and dropped: at 51 MB it was 40% of the APK, and the ncnn `Vits` is filesystem-only so it would also have cost a second 51 MB extracted copy on disk.

So a clean install transcribes with no network, but speaking still needs one voice download.

Notably **offline after first launch**: `astronomy`, `calendar`, `clock`, `contacts` (only `libphonenumber` locally), `files`, `notes` (Room + Ink), `things`, `camera` (on-device `tasks-vision:0.10.14` BlazeFace), `pdf` (Rust `libpdf_render.so`), `music` (Media3 ExoPlayer + local scan), `games/*`, `passwords` (biometric + KeePass), `library/*`, `health` (including food search, which ships its database in the APK).
...
| `com.microsoft.onnxruntime:onnxruntime-android` | `1.27.0` | `openassistant` (SigLIP), `speech` (whisper-tiny int8) | MIT, Microsoft-maintained, low risk but native `.so` (~27 MB arm64, 35% of the `speech` APK). The reduced `onnxruntime-mobile` package is not usable: it lacks `ConvInteger`, which whisper's quantized Conv1d stem needs, and its last release was 1.18.0 in May 2024 | `openassistant/build.gradle.kts`, `speech/build.gradle.kts`, `libs.versions.toml:85` |
