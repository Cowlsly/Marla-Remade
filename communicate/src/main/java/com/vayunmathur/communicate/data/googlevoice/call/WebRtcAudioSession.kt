...
/**
 * WebRTC audio leg of a Google Voice call.
 *
 * Wraps a single [PeerConnection] carrying one Opus audio m-line (with PCMU/PCMA/G722/CN and
 * RFC 4733 `telephone-event` offered too, matching the SDP observed in `voice-documentation.md`
 * → "Media (WebRTC / SDP)"). SDP is exchanged as full offers/answers over SIP, so we gather
...
class WebRtcAudioSession(private val appContext: Context) {
    ...
    private fun addLocalAudio(f: PeerConnectionFactory) {
        ...
        val track = f.createAudioTrack("gv_audio", source)
        audioSource = source
        localAudioTrack = track
        audioSender = peerConnection?.addTrack(track, listOf("gv_stream"))
    }
    ...
    /** Apply the remote answer SDP received in the SIP 200 OK. */
    ...
    suspend fun createAnswer(remoteOfferSdp: String): String {
        ...
        val answer = pc.createAnswerSuspend(constraints)
