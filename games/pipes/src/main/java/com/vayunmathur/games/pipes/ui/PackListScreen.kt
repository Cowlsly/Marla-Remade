...
fun PackListScreen(packs: List<PackProgress>, onOpenPack: (Int) -> Unit, onOpenSettings: () -> Unit, onOpenGameCenter: () -> Unit, daily: DailyProgress? = null, onOpenDaily: () -> Unit = {}) {
    DetailLazyColumn(title = stringResource(R.string.pack_selector), actions = { IconButton(onClick = onOpenSettings) { IconSettings() }; IconButton(onClick = onOpenGameCenter) { Icon(painterResource(id = android.R.drawable.btn_star_big_on), "Achievements") } }, scrollBehavior = appBarScrollBehavior()) {
