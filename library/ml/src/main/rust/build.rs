...
//!
//! Copied from `library/map/src/main/rust/build.rs`, which already globbed `.comp`
//! alongside `.vert`/`.frag` and already targeted `vulkan1.1`. Only the doc comments
//! and the final panic message differ.
//!
...
//! part of the runtime, and an asset can be missing at run time while a linked
//! `&[u8]` cannot.
...
fn find_glslc() -> PathBuf {
    ...
    panic!(
        "glslc not found. Looked for ndk/*/shader-tools/{host}/{executable} under \
         {sdks:?} and {ndks:?}, then on PATH. Install the NDK, or set GLSLC. Every \
         layer of both networks is a compute shader, so this build stops here rather \
         than producing a runtime that reports itself unavailable on every device.",
    );
}
